...
func getIznos(f models.Faktura) int {
	var iznos int
	for j := 0; j < len(f.Podaci.Stavke); j++ {
		iznos = iznos + (f.Podaci.Stavke[j].Kom * 555) //f.Stavke[j].Cena)
	}
	return iznos
}
